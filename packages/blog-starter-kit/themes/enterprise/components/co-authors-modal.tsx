        ...
        <div className="flex flex-wrap items-center overflow-hidden">
          ...
          <div className="flex flex-row items-center text-clip">
            <a
              ...
              title={author.name}
              className="truncate font-sans text-sm font-medium text-slate-200"
            >
              {author.name}
            </a>
          </div>
    ...
    <DialogPrimitive.Root open>
      <DialogPrimitive.Portal>
        <DialogPrimitive.Overlay
          onClick={closeModal}
          className="fixed inset-0 z-50  opacity-50 transition-opacity duration-300 ease-out bg-slate-600"
        />
        <DialogPrimitive.Content
          onEscapeKeyDown={closeModal}
          className="fixed bottom-0 left-0 right-0 z-50 flex w-full max-w-[1200px] flex-col items-center overflow-hidden rounded-b-none rounded-t-lg   border-slate-800 bg-slate-900 text-slate-50 md:bottom-50 md:left-50 md:w-96 md:-translate-x-1/2 md:translate-y-1/2 md:rounded-xl lg:w-108 xl:w-116"
        >
          <DialogPrimitive.DialogTitle className="w-full px-6 py-4 text-lg font-semibold">
            Authors in this article
          </DialogPrimitive.DialogTitle>
          <DialogPrimitive.Close className="absolute right-2 top-4 text-slate-50" asChild>
          <Button
                type="outline"
                label=""
                icon={<CloseSVG className="h-5 w-5 fill-current" />}
                className="rounded-xl !border-transparent !px-3 !py-2 hover:bg-neutral-800 text-white"
                onClick={closeModal}
            />
