	...
	const searchResultsList = searchResults.map((post) => {
		...
		return (
			<Link
				key={post.id}
				href={postURL}
				className="flex flex-row items-center gap-5 px-4 py-2 hover:bg-slate-50 focus:outline-1"
			>
	...
	return (
		<div className="relative col-span-1">
			<input
				...
				className="w-full rounded-full border border-slate-400 bg-slate-100 px-4 py-2 text-base focus:bg-transparent"
			/>
			{query && (
				<>
					{isSearching && (
						<div className="top-100 absolute left-0 z-10 mt-1 flex w-full flex-col items-stretch overflow-hidden rounded-lg border bg-white p-1 text-left text-slate-900 shadow-2xl">
