...
export const SubscribeForm = () => {
	...
	return (
		<>
			{!status && (
				<div className="relative w-full rounded-full bg-neutral-950 p-2">
					<input
						ref={inputRef}
						type="email"
						placeholder="john@doe.com"
						className="focus:outline-primary-500 left-3 top-3 w-full rounded-full p-3 text-base  outline-none bg-neutral-950 text-neutral-50"
					/>
					<button
						disabled={requestInProgress}
						onClick={subscribe}
						className="bg-primary-600 absolute right-3 top-3 rounded-full px-3 py-2 text-white disabled:cursor-not-allowed disabled:opacity-80"
					>
						Subscribe
					</button>
				</div>
			...
			{status === 'PENDING' && (
				<div className="relative w-full p-2 text-center">
					<p className="font-bold text-green-500">Almost there!</p>
