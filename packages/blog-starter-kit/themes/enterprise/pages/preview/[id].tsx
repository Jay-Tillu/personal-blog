...
export default function Post({ publication, post }: Props) {
	...
	const tagsList = post.tags?.map((tag) => (
		<li key={tag.id}>
			<Link
				href={`/tag/${tag.slug}`}
				className="block rounded-full border px-2 py-1 font-medium  border-neutral-800 hover:bg-neutral-800 md:px-4"
			>
				#{tag.slug}
			</Link>
		</li>
			...
			<Layout>
				...
				<Container className="pt-10">
					<article className="flex flex-col items-start gap-10 pb-10">
						...
						<div className="mx-auto w-full px-5  text-neutral-300 md:max-w-screen-md">
